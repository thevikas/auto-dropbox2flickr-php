...
class phpFlickr {
	...
	*/
	...
	function auth2 ($perms = "read", $remember_uri = true) {
		// Redirects to Flickr's authentication piece if there is no valid token.
		// If remember_uri is set to false, the callback script (included) will
		// redirect to its default page.

                $frob = $this->auth_getFrob();

		if (empty($_SESSION['phpFlickr_auth_token']) && empty($this->token)) {
			$api_sig = md5($this->secret . "api_key" . $this->api_key . "frob" . $frob . "perms" . $perms);
			$url = "http://www.flickr.com/services/auth/?api_key=" . $this->api_key . "&frob=" . $frob . "&perms=" . $perms . "&api_sig=". $api_sig;
			$rt = array();
			$rt['url'] = $url;
			$rt['frob'] = $frob;
			return $rt;
